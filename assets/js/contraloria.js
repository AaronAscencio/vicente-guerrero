fetch('https://www.ndim.com.mx/contraloria/get-contraloria/195/')
  .then(response => response.json())
  .then(data => {
    // Obtener el contenedor de resultados
    const resultadosContainer = document.getElementById('id_contraloria');
    ...
    for (const anio in data) {
      ...
      for (const periodo in periodos) {
        ...
        const contraloria = periodos[periodo];

        // Iterar sobre las fracciones
        for (const registro of contraloria) {
